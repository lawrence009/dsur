...
scatter + geom_point()
imgFile <- paste(imgDir,"06 Advert Scatter Basic.png", sep="/")
ggsave(file = imgFile)

scatter +
    geom_point(size = 3) +
    labs(x = "Adverts", y = "Packets") +
    scale_y_continuous(limits=c(0, 15), breaks=0:15) +
    scale_x_continuous(limits=c(0, 9), breaks=0:9)
imgFile <- paste(imgDir,"06 Advert Scatter Nice.png", sep="/")
ggsave(file = imgFile)
...
cor(examData[, c(3, 4)], use = "complete.obs", method = "pearson")
...
#-------------------------Smart Alex Task 1
#Load the data:
essayData <- read.delim("data/EssayMarks.dat",  header = TRUE)
...
cor.test(as.numeric(chickFlick$gender), chickFlick$arousal)

cor.test(as.numeric(chickFlick$film), chickFlick$arousal)

#----Smart Alex Task 3----
#load in the data
gradesData <- read.csv("data/grades.csv", header = TRUE)
